...
internals.getTasksForListClosure = function(list, userid) {
    return function(done) {
        {{projectVariableName}}.Tasks.getAll({ user: userid, list: list.id }, function(err, tasks) {
            done(err, tasks);
        });
    };
};
...
{{name}}.add = function(input, callback) {
    var errorMessages = [];
    //validate fields
    if (!input.user) {
        errorMessages.push('{{name}}.add: Object field "user" is required.');
    }
    if (!input.name) {
        errorMessages.push('{{name}}.add: Object field "name" is required.');
    }
    ...
    if (errorMessages.length > 0) {
        ...
        {{projectVariableName}}.Async.parallel({
            user: function(cb) {
...
{{name}}.activate = function(input, callback) {
    var errorMessages = [];
    //test existence of fields
    if (!input.id) {
        errorMessages.push('{{name}}.activate: Object field "id" is required.');
    }
    if (!input.user) {
        errorMessages.push('{{name}}.activate: Object field "user" is required.');
    }
    input.id = parseInt(input.id);
    
    //validate fields
    if (!internals.isValidID(input.id)) {
        errorMessages.push('{{name}}.activate: Object field "id" must be an integer.');
    }
...
{{name}}.inactivate = function(input, callback) {
    var errorMessages = [];
    //test existence of fields
    if (!input.id) {
        errorMessages.push('{{name}}.inactivate: Object field "id" is required.');
    }
    if (!input.user) {
        errorMessages.push('{{name}}.inactivate: Object field "user" is required.');
    }
    input.id = parseInt(input.id);
    
    //validate fields
    if (!internals.isValidID(input.id)) {
        errorMessages.push('{{name}}.inactivate: Object field "id" must be an integer.');
    }
...
{{name}}.delete = function(input, callback) {
    var errorMessages = [];
    //test existence of fields
    if (!input.id) {
        errorMessages.push('{{name}}.delete: Object field "id" is required.');
    }
    if (!input.user) {
        errorMessages.push('{{name}}.delete: Object field "user" is required.');
    }
    input.id = parseInt(input.id);
    
    //validate fields
    if (!internals.isValidID(input.id)) {
        errorMessages.push('{{name}}.delete: Object field "taskid" must be an integer.');
    }
...
{{name}}.update = function(input, callback) {
    ...
    if (!input.id) {
        errorMessages.push('{{name}}.update: Object field "id" is required.');
    }
    if (!input.user) {
        errorMessages.push('{{name}}.update: Object field "user" is required.');
    }
    input.id = parseInt(input.id);
    
    //validate fields
    if (!internals.isValidID(input.id)) {
        errorMessages.push('{{name}}.update: Object field "id" must be an integer.');
    }
...
{{name}}.get = function(input, callback) {
    var errorMessages = [];
    //test existence of required fields
    if (!input.id) {
    ...
    if (!input.user) {
        errorMessages.push('{{name}}.get: Object field "user" is required.');
    }
    ...
    if (errorMessages.length > 0) {
        ...
        'LEFT OUTER JOIN `Shared{{name}}` AS `SharedUsers.SharedList` ON `List`.`id` = `SharedUsers.SharedList`.`ListId` '+
        ...
        internals.data.sequelize.query(query, internals.data.List, {raw:true}, [input.id,input.user,input.user]).success(function(list){
            ...
            if (list != null && list.length > 0) {
                ...
                {{name}}.getSharedUsers({ id: input.id }, function(err, sharedUsers) {
                    output.sharedUsers = sharedUsers || [];
                    {{projectVariableName}}.Tasks.getAll({ user: input.user, list: input.id }, function(err, tasks) {
                        output.tasks = tasks;
                        callback(null, output);
                    });
                });
...
{{name}}.getAll = function(input, callback) {
    var errorMessages = [];
    //test existence of required fields
    if (!input.user) {
        errorMessages.push('{{name}}.getAll: Object field "user" is required.');
    }
    ...
    if (errorMessages.length > 0) {
        ...
        internals.data.List.findAll({ 
            ...
            {{projectVariableName}}.Async.parallel(internals.getAllParallelClosure(lists, input.user), function(err, results) {
